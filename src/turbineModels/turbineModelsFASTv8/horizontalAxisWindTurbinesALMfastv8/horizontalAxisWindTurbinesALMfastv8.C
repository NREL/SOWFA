...
void horizontalAxisWindTurbinesALMfastv8::readInput()
{
...
void horizontalAxisWindTurbinesALMfastv8::sendInput()
{
    // Set global inputs to FAST.
    ...
    if (p < numTurbines)
    {
        ...
        baseLoc[0] = baseLocation[p].x();
        baseLoc[1] = baseLocation[p].y();
        baseLoc[2] = baseLocation[p].z();

        FAST.setTurbineData
        (
            p,
            FASTInputFileName[p],
            FASTRestartFileName[p],
            baseLoc,
            numBladePoints[p],
            numTowerPoints[p]
        );
...
void horizontalAxisWindTurbinesALMfastv8::initializeArrays()
{
...
void horizontalAxisWindTurbinesALMfastv8::updateRotorSearchCells(int turbineNumber)
{
...
void horizontalAxisWindTurbinesALMfastv8::updateNacelleSearchCells(int turbineNumber)
{
    // Define the cells that can possibly be influenced by the force
...
void horizontalAxisWindTurbinesALMfastv8::updateTowerSearchCells(int turbineNumber)
{
    // Define the cells that can possibly be influenced by the force
...
void horizontalAxisWindTurbinesALMfastv8::updateTurbinesControlled()
{
...
void horizontalAxisWindTurbinesALMfastv8::updateBladePointControlProcNo()
{
...
void horizontalAxisWindTurbinesALMfastv8::updateNacellePointControlProcNo()
{
    // Create a local and global list of minimum distance cells to velocity sampling 
    // points of turbines that this processor controls.  Initialize the values to huge.
...
void horizontalAxisWindTurbinesALMfastv8::updateTowerPointControlProcNo()
{
    // Create a local and global list of minimum distance cells to velocity sampling 
    // points of turbines that this processor controls.  Initialize the values to huge.
...
void horizontalAxisWindTurbinesALMfastv8::updateRadius(int turbineNumber)
{
...
void horizontalAxisWindTurbinesALMfastv8::getPositions()
{
   ...
   for(int i = 0; i < numTurbines; i++)
   {
      ...
      if (p == i) 
      {
         FAST.getHubShaftDirection(shaftOrientation);
   ...
   if (p < numTurbines)
   {
      ...
      {
         FAST.getVelNodeCoordinates(pointLocation,i);
   ...
   if (p < numTurbines)
   {
      ...
      {
          FAST.getForceNodeCoordinates(pointLocation,i);
...
void horizontalAxisWindTurbinesALMfastv8::sendVelocities()
{
   ...
   if (p < numTurbines)
   {
        ...
        FAST.setVelocity(pointVelocity, m);
        ...
        forAll(bladeWindVectors[i], j)
        {
            ...
            {
                 pointVelocity[0] = bladeWindVectors[i][j][k].x();
                 pointVelocity[1] = bladeWindVectors[i][j][k].y();
                 pointVelocity[2] = bladeWindVectors[i][j][k].z();
...
void horizontalAxisWindTurbinesALMfastv8::getForces()
{
...
void horizontalAxisWindTurbinesALMfastv8::getNumBlades()
{
   // Zero the list.
   forAll(numBl,i)
   {
...
void horizontalAxisWindTurbinesALMfastv8::getNumBladePoints()
{
   // Zero the list.
...
void horizontalAxisWindTurbinesALMfastv8::getNumTowerPoints()
{
   // Zero the list.
...
void horizontalAxisWindTurbinesALMfastv8::getChordLengths()
{
   // Get the total number of force points for this processor's turbine.
   int localNumPoints = 0;
   if (p < numTurbines)
   ...
   if (p < numTurbines)
   {
       ...
       for (int i = 0; i < localNumPoints; i++)
       {
           chord_[startIndex + i] = FAST.getChord(i);
       }
   }
...
void horizontalAxisWindTurbinesALMfastv8::computeBladeAlignedVectors()
{
    forAll(bladePoints,i)
    {
        forAll(bladePoints[i],j)
...
void horizontalAxisWindTurbinesALMfastv8::sampleBladePointWindVectors()
{
...
void horizontalAxisWindTurbinesALMfastv8::sampleTowerPointWindVectors()
{
...
void horizontalAxisWindTurbinesALMfastv8::sampleNacellePointWindVectors()
{
...
void horizontalAxisWindTurbinesALMfastv8::updateBladeBodyForce(int turbineNumber)
{
    int i = turbineNumber;
        ...
        if (bladeInfluenceCells[i].size() > 0)
        {
            ...
            forAll(bladePointForce[i], j)
            {
                ...
                forAll(bladePointForce[i][j], k)
                {
                    ...
                    rotorTorque[i] += (bladePointForce[i][j][k] * r) & bladeAlignedVectors[i][j][k][1];
                    ...
                    forAll(bladeInfluenceCells[i], m)
                    {
                        ...
                        if (dis <= bladeProjectionRadius[i])
                        {
                            ...
                            if (bladeForceProjectionDirection[i] != "localVelocityAlignedCorrected")
                            {
                               ...
                               rotorTorqueBodySum += (bodyForce[bladeInfluenceCells[i][m]] * r * mesh_.V()[bladeInfluenceCells[i][m]]) & bladeAlignedVectors[i][j][k][1];
  ...
  //}
    ...
    reduce(rotorAxialForce[i],maxOp<scalar>());
    reduce(rotorTorque[i],maxOp<scalar>());
    ...
    Info << "Turbine " << i << tab << "Rotor Axial Force from Body Force = " << rotorAxialForceBodySum << tab << "Rotor Axial Force from Actuator = " << rotorAxialForce[i] << tab  
         << "Ratio = " << rotorAxialForceBodySum/max(rotorAxialForce[i],1.0E-5) << endl;
...
void horizontalAxisWindTurbinesALMfastv8::updateTowerBodyForce(int turbineNumber)
{
    int i = turbineNumber;
  ...
  //}
    ...
    Info << "Turbine " << i << tab << "Tower Axial Force from BodyForce = " << towerAxialForceBodySum << tab << "Tower Axial Force from Actuator = " << towerAxialForceSum << tab
         << "Ratio = " << towerAxialForceBodySum/max(towerAxialForceSum,1.0E-5) << endl;
}



void horizontalAxisWindTurbinesALMfastv8::updateNacelleBodyForce(int turbineNumber)
{
    int i = turbineNumber;
  ...
  //}
    ...
    // Print information comparing the actual tower thrust to the integrated body force.
    Info << "Turbine " << i << tab << "Nacelle Axial Force from BodyForce = " << nacelleAxialForceBodySum << tab << "Nacelle Axial Force from Actuator = " << nacelleAxialForceSum << tab
         << "Ratio = " << nacelleAxialForceBodySum/max(nacelleAxialForceSum,1E-5) << endl;
}



scalar horizontalAxisWindTurbinesALMfastv8::computeBladeProjectionFunction(vector disVector, int turbineNumber, int bladeNumber, int elementNumber)
{
    int i = turbineNumber;
    int j = bladeNumber;
    int k = elementNumber;
...
void horizontalAxisWindTurbinesALMfastv8::update()
{
    ...
    if(actuatorUpdateType[0] == "oldPosition")
    {
      ...
      //yawNacelle();
        ...
        List<vector> mainShaftOrientationOld = 1.0*mainShaftOrientation;
        List<vector> rotorApexOld = 1.0*rotorApex;
        ...
        forAll(turbineName,i)
        {
            mainShaftOrientationChange.append(Foam::mag(mainShaftOrientation[i] - mainShaftOrientationOld[i]));
            rotorApexChange.append(Foam::mag(rotorApex[i] - rotorApexOld[i]));
         
            if ((mainShaftOrientationChange[i] > 1E-5) || (rotorApexChange[i] > 1E-1))
            {
    ...
    else if(actuatorUpdateType[0] == "newPosition")
    {
      ...
      //yawNacelle();
        ...
        forAll(turbineName,i)
        {
            ...
            Info << "Turbine " << i << " mainShaftOrientationChange = " << mainShaftOrientationChange[i] << endl;
            Info << "Turbine " << i << " rotorApexChange = " << rotorApexChange[i] << endl;
