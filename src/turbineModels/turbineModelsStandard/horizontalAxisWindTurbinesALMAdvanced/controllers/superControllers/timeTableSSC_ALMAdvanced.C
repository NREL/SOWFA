...
void readInputSSC_ALMAdvanced(char *inputFile, int nTurbs, int nControlVars,
...
void lookupControlAction_ALMAdvanced(float simTime, int nTurbs, int nControlVars, std::vector< std::vector<float> >&timeArray, 
...
void SC_timeTable_ALMAdvanced(int nTurbs,int nControlVars, std::vector< std::vector<float> >&nextControlAction, float simTime)
{
	...
	if(isFirstCall == 1){
		readInputSSC_ALMAdvanced(inputFile, nTurbs, nControlVars, nTurbInputs, timeArray, controlArray);
		isFirstCall = 0; // disable for future calls
	}
	
	// Retrieve control settings for time instant 'simTime'
	lookupControlAction_ALMAdvanced(simTime, nTurbs, nControlVars, timeArray,controlArray, nTurbInputs,nextControlAction);
...
int main()
{
	...
	simTime = 5.0;
	SC_timeTable_ALMAdvanced(nTurbs,nOutputsFromSSC,nextControlAction,simTime);
