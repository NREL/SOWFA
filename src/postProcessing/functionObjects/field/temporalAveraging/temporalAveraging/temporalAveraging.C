...
void Foam::temporalAveraging::resetFields()
{
    forAll(faItems_, i)
    {
        if (faItems_[i].mean())
...
void Foam::temporalAveraging::initialize()
{
    resetFields();
    ...
    forAll(faItems_, fieldI)
    {
        ...
        {
            WarningIn("void Foam::temporalAveraging::initialize()")
...
void Foam::temporalAveraging::calcAverages()
{
    if (!initialised_)
    {
        initialize();
...
void Foam::temporalAveraging::writeAverages() const
{
...
void Foam::temporalAveraging::writeAveragingProperties() const
{
    IOdictionary propsDict
    (
        IOobject
        (
            "temporalAveragingProperties",
            obr_.time().timeName(),
            "uniform",
            obr_,
            IOobject::NO_READ,
...
void Foam::temporalAveraging::readAveragingProperties()
{
    totalIter_.clear();
...
Foam::temporalAveraging::temporalAveraging
(
    const word& name,
    ...
    totalTime_()
{
    ...
    else
    {
        ...
        WarningIn
        (
            "temporalAveraging::temporalAveraging"
            "("
                "const word&, "
...
void Foam::temporalAveraging::read(const dictionary& dict)
{
    if (active_)
    {
        initialised_ = false;
...
void Foam::temporalAveraging::execute()
{
    if (active_)
    {
        calcAverages();
...
void Foam::temporalAveraging::end()
{
    if (active_)
    {
        calcAverages();
...
void Foam::temporalAveraging::timeSet()
{}


void Foam::temporalAveraging::write()
{
    if (active_)
    {
        writeAverages();
...
void Foam::temporalAveraging::updateMesh(const mapPolyMesh&)
{
    // Do nothing
}


void Foam::temporalAveraging::movePoints(const polyMesh&)
{
    // Do nothing
}
