MODULE SysSubs
   ...
   ! 
   ! +--------------------------------------------------------------------------------+
   ! | Changes for gfortan 4.4.4                                            10/2/2010 |
   ! |                                                                                |
   ! | Change FORM='BINARY' to FORM="UNFORMATTED" in OPEN statement                   |
   ! | CARRIAGECONTROL='FORTRAN' removed from OPEN statement (not implemented)        |
   ! | GETARG(POS,VALUE), two parameters rather than three                            |
   ! | COMMAND_ARGUMENT_COUNT intrinsic to gfotran                                    |
   ! | Increase StatArray(12) size in FileSize to 13                                  |
   ! | GET_COMMAND intrinsic but has different number of arguments                    |
   ! | GET_COMMAND_ARGUMENT intrinsic to gfortran                                     |
   ! | GET_ENVIRONMENT_VARIABLE intrinsic but intrisically it is a subroutine but here|
   ! |    it is used as a Function                                                    |
   ! | change CU = 6  console UNIT number (default OPEN)                              |
   ! +--------------------------------------------------------------------------------+

   ! It contains the following routines:
   ! OK = tested    NT = not tested
   !     OK FUNCTION    COMMAND_ARGUMENT_COUNT()
   !     OK SUBROUTINE  FileSize ( FileName, Size )
   !     NT SUBROUTINE  FindLine ( Str , MaxLen , StrEnd )
   !     NT SUBROUTINE  FlushOut ( Unit )
   !     OK SUBROUTINE  Get_Arg ( Arg_Num , Arg , Error )                                   ! Please use GET_COMMAND_ARGUMENT() instead.
   !     OK SUBROUTINE  Get_Arg_Num ( Arg_Num )                                             ! Please use COMMAND_ARGUMENT_COUNT() instead.
   !     OK SUBROUTINE  GET_COMMAND ( Command, Length, Status )
   !     OK SUBROUTINE  GET_COMMAND_ARGUMENT ( Number, Value, Length, Status )
   !     NO SUBROUTINE  GET_CWD( DirName, Status )
   !     OK FUNCTION    Get_Env( EnvVar )                                                   ! Please use GET_ENVIRONMENT_VARIABLE() instead.
   !     OK FUNCTION    GET_ENVIRONMENT_VARIABLE( Name, Value, Length, Status, Trim_Name )
   !     OK FUNCTION    Is_NaN( DblNum )
   !     OK SUBROUTINE  OpenBinFile ( Un, OutFile, RecLen, Error )
   !     OK SUBROUTINE  OpenBinInpFile( Un, InFile, Error )
   !     OK SUBROUTINE  OpenUnfInpBEFile ( Un, InFile, RecLen, Error )
   !     OK SUBROUTINE  OpenCon
   !     OK SUBROUTINE  ProgExit ( StatCode )
   !     OK but SUBROUTINE  UsrAlarm                                 ! No alarm but compiles
   !     OK FUNCTION    UserTime()                                   ! Removed: Replace by F95 intrinsic, CPU_TIME().
   !     OK SUBROUTINE  WrNR( Str )
   !     OK but SUBROUTINE  WrOver ( Str )                           ! Works but doesnt overwrite
   !     OK SUBROUTINE  WrScr ( Str )                                ! but not fully checked
...
!=======================================================================
   ...
   INTEGER                      :: CU       = 6                                 ! The I/O unit for the console.
...
!=======================================================================
   ...
   Arg_Num = COMMAND_ARGUMENT_COUNT()                                 ! Use intrinsic function
...
!=======================================================================
   ...
   ! Local Variables
   CHARACTER(160)               :: Name             ! Linux needs long character string here

   CALL GETCWD(Name,Status)
   DirName = TRIM(Name)

   CALL GETCWD(DirName, Status)
...
!=======================================================================
   ...
   OPEN( Un, FILE=TRIM( OutFile ), STATUS='UNKNOWN', FORM='UNFORMATTED' , ACCESS='SEQUENTIAL', RECL=RecLen , IOSTAT=IOS )
...
!=======================================================================
   ...
   OPEN( Un, FILE=TRIM( InFile ), STATUS='OLD', FORM='UNFORMATTED', IOSTAT=IOS, ACTION='READ' )
...
!=======================================================================
   ...
   ! Console Unit=6 open by default in gfortran, change above and leave
   !   this as a blank routine
...
!=======================================================================
   ...
   ! +--------------------------------------------------------------------+
   ! | Cant figure out how to make gfortran sound an alarm, leave blank   |
   ! +--------------------------------------------------------------------+
...
!=======================================================================
   ...
   ! +----------------------------------------------------------------+
   ! | Cant find a clean way to overwrite a terminal line. Just use   |
...
!=======================================================================
   ...
   ! This routine writes out a (long) string to the screen.
   ...
   INTEGER                      :: Indent                                       ! The amount to be indented.
