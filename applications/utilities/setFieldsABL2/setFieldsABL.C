...
scalar flm0(setFieldsABLDict.lookupOrDefault<scalar>("flm0",2.56E-6));
scalar fmm0(setFieldsABLDict.lookupOrDefault<scalar>("fmm0",1.00E-4));
...
if (updateBoundaryFields)
{
    ...
    flm.correctBoundaryConditions();
    fmm.correctBoundaryConditions();
}
...
Info<< "Writing field flm" << endl;
flm.write();
Info<< "Writing field fmm" << endl;
fmm.write();
